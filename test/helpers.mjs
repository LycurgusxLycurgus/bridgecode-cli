...
export async function writeManifest(root,name="@bridgecode/cli",version="4.3.3"){
...
export async function packageSnapshot(t,version="4.3.0"){
 const root=await fixture(t,"bridgecode-snapshot-");
 const old=JSON.parse(await readFile(path.join(PACKAGE_ROOT,"legacy/"+version+".json"),"utf8"));
 if(sha256(old.hook)!==old.hookHash)throw new Error(version+" hook snapshot mismatch");
 ...
 await writeFile(path.join(root,"package.json"),JSON.stringify({name:"@bridgecode/cli",version}));
 await writeManifest(root,"@bridgecode/cli",version);return root;
}
